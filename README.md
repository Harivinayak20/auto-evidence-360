...
- **Measured entity resolution:** exact make/model/year reference-match coverage ranges from 15.47% to 64.86% across operational sources. Unresolved names go to a review queue rather than a hidden fuzzy join.
...
flowchart LR
    ...
    D --> H["Unresolved alias review queue"]
...
| `src/fabric/notebooks/` | Bronze, Silver/entity-resolution, and Gold PySpark notebook source |
...
2. Upload the eight files in `data/fabric_upload/` to `Files/landing/auto_evidence_360/`.
3. Create Fabric notebooks from `src/fabric/notebooks/01_ingest_bronze.py`, `02_conform_vehicle_entities.py`, and `03_build_gold.py`.
4. Run them in numerical order. A row-count, schema, or referential-integrity failure stops publication.
