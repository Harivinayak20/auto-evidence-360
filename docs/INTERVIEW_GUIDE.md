...
> Auto Evidence 360 combines seven real federal vehicle datasets in Microsoft Fabric and Power BI. I built a governed pipeline that validates every source, removes sensitive complaint fields, standardizes vehicle identities, and creates a review queue with a plain-language reason. The dashboard helps a used-car team decide what deserves deeper review, but it deliberately does not claim a universal reliability score.
...
>
> In Fabric, Bronze preserves each approved extract and fails if the manifest does not match. Silver types and deduplicates the records, classifies official text into explainable topics, and creates a normalized make/model/year key. Exact reference matching varies a lot by source, so unmatched names go into an alias review queue rather than being silently joined. Gold publishes a star schema, source-specific facts, an evidence mart, and a transparent review-priority queue.
>
> Power BI answers why a vehicle needs review and lets the user drill into the source evidence. The key analytical guardrail is that complaint and bulletin counts are signals, not reliability rates, because we do not have a make/model/year exposure denominator.
...
| Direct Lake | Power BI reads Fabric tables without another imported copy |
...
> The raw row count is often the wrong metric. A recall campaign repeats for different models, years, and components, and its potential-unit estimate can repeat too. My measure counts distinct campaigns and takes one estimate per campaign before summing. The same discipline applies to complaint reports, service documents, NCAP variants, and EPA configurations.
...
The source configuration records exact URLs and assumptions; downloads write metadata and hashes; the Fabric upload manifest records source and output hashes, schemas, and row counts; the notebook is executable; and Bronze/Gold gates stop inconsistent publication.
